...
resourcename="enchiridion"  # lower case, only
resourcegroup="enchiridion_rg"
...
# git clone https://github.com/apilogicserver/enchiridion.git
# cd enchiridion
...
az group create --name enchiridion_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group enchiridion_rg --sku S1 --is-linux
az container create --resource-group enchiridion_rg --name enchiridion --image apilogicserver/enchiridion:latest --dns-name-label enchiridion --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//enchiridion.westus.azurecontainer.io:5656'
